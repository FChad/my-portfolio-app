...
import type { DocNote } from '~/composables/useDocumentation'
        ...
        <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-6">
                <div v-for="note in notes" :key="note.id"
                    class="group bg-white/60 dark:bg-gray-800/60 backdrop-blur-sm rounded-xl sm:rounded-2xl md:rounded-3xl p-4 sm:p-5 md:p-6 shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1">
                    <div class="flex items-center gap-2.5 sm:gap-3 md:gap-4 mb-3 sm:mb-4">
                        <div :class="[
                            'w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-lg sm:rounded-xl md:rounded-2xl flex items-center justify-center shadow-lg group-hover:scale-110 group-hover:rotate-3 transition-all duration-300',
                            getColorClasses(note.color).bg
                        ]">
                            <Icon :name="note.icon" class="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 text-white" />
                        </div>

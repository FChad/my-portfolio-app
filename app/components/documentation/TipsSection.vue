    ...
    <section class="py-12 sm:py-16 md:py-24 relative overflow-hidden">
        ...
        <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="text-center mb-8 sm:mb-12 md:mb-16">
                <h2
                    class="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black text-neutral-800 dark:text-white mb-3 sm:mb-4">
                    {{ title }}
                </h2>
                ...
                <div v-for="item in items" :key="item.id"
                    ...
                    <button @click="toggleExpanded(item.id)"
                        ...
                        <Icon :name="isExpanded[item.id] ? 'mdi:chevron-up' : 'mdi:chevron-down'"
                            class="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-neutral-400 transition-transform shrink-0 ml-2 sm:ml-4" />
                    </button>

                    <Transition name="expand">
                        <div v-show="isExpanded[item.id]" class="px-4 sm:px-5 md:px-6 pb-4 sm:pb-5 md:pb-6">
                            <div
                                class="pt-3 sm:pt-4 border-t border-neutral-200 dark:border-neutral-700 space-y-3 sm:space-y-4">

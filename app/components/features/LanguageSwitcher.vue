...
<template>
    <div class="relative">
        ...
        <button @click.stop="toggleDropdown"
            class="cursor-pointer flex items-center gap-2 px-3 py-2 rounded-lg border border-neutral-300 dark:border-neutral-600 bg-neutral-50 dark:bg-neutral-900 hover:bg-neutral-200 dark:hover:bg-neutral-800"
            :aria-expanded="isOpen" aria-haspopup="true">
            <Icon :name="getFlag(locale)" class="w-5 h-5" />

            <Icon name="mdi:chevron-down" class="w-4 h-4 text-neutral-700 dark:text-neutral-300 transition-transform"
                :class="{ 'rotate-180': isOpen }" />
        </button>

        <!-- Dropdown Menu -->
        <div v-show="isOpen"
            class="absolute top-full right-0 mt-1 flex flex-col gap-1 bg-neutral-50 dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-600 rounded-lg shadow-lg z-[60] p-2">
